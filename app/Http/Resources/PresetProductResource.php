...
class PresetProductResource extends JsonResource
{
    public function toArray($request)
    {
        ...
        return [
            ...
            'present' => $this->present, // 대표 출고 상품여부 (여러개 시켜도 시안을 하나만 작성한다그래서 만든 기능)
            ...
            'preset' => [
                ...
                'format_options' => $this->preset->format_options,
                'count_preset_product' => $this->preset->presetProducts()->where('additional', 0)->count(),
            ],

            'product' => [
                'id' => $product->id,
                'img' => $product->img ?? '', // 대표이미지
                'format_categories' => Arr::getArrayToString($product->categories->pluck('title')->toArray()),
            ...
            'format_type_request' => $this->type_request ? TypeRequest::getLabel($this->type_request) : '',
            ...
            'admin_can_confirm' => $this->admin_can_confirm,
            'state_prototype' => $this->state_prototype,
            'format_state_prototype' => StatePrototype::getLabel($this->state_prototype),

            'has_uncheck_feedback' => $this->has_uncheck_feedback,
            'need_alert_delivery' => $this->need_alert_delivery,

            'format_updated_at' => $this->updated_at ? Carbon::make($this->updated_at)->format("Y.m.d") : '',
